...
class Devices(object):
    ...
    # Reg. L1
    tbx_reg1 = ModbusTCPDevice('192.168.10.10', port=502, unit_id=1, timeout=5.0, refresh=1.0)
...
class TabRegL1(HMITab):
    def __init__(self, notebook, update_ms=500, *args, **kwargs):
        ...
        self.cons_r.build()
        # Etats PID
        self.frmEtatPid = tk.LabelFrame(self, text='Etats PID', padx=10, pady=10)
        ...
        self.frmCmdReg.grid(row=2, column=0, padx=5, pady=5, sticky=tk.NSEW)
        self.cmd_l = HMIButtonList(self.frmCmdReg, btn_args={'width': 15}, grid_args={'pady': 5})
        self.cmd_l.add('Régulateur en auto', tag_valid=Tags.REG1_LOCAL, cmd=lambda: Tags.REG1_MODE_AUTO.set(True),
                       btn_args={'bg': GREEN})
        self.cmd_l.add('Régulateur en manu', tag_valid=Tags.REG1_LOCAL, cmd=lambda: Tags.REG1_MODE_MANU.set(True),
                       btn_args={'bg': RED})
        self.cmd_l.build()
        # Mesures régulateur
        ...
        self.mes_r.add('Mesure débit', Tags.REG1_DEF_M_DEBIT, unit='nm3/h', fmt='%0.f')
        self.mes_r.add('Sortie régulateur', Tags.REG1_OUT_REG, unit='%', fmt='%0.2f')
        self.mes_r.add('PID Wobbe', Tags.REG1_OUT_REG, unit='%', fmt='%0.2f')
        self.mes_r.add('PID PCS', Tags.REG1_OUT_REG, unit='%', fmt='%0.2f')
        self.mes_r.add('PID débit', Tags.REG1_OUT_REG, unit='%', fmt='%0.2f')
        self.mes_r.build()
