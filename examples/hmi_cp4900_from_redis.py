...
class Tags(object):
    ...
    CP_STATE = Tag(0, src=Devices.redis, ref={'type': 'str', 'key': 'cp4900:state'})
    RPI_SIGFOX_SINCE_TX = Tag(0, src=Devices.redis, ref={'type': 'int', 'key': 'cp4900:rpi_sigfox:sec_since_tx'})
...
class TabMisc(HMITab):
    def __init__(self, notebook, update_ms=500, *args, **kwargs):
        ...
        self.val_list.add('THT', Tags.CP_THT, unit='mg/nm3', fmt='%.2f')
        self.val_list.add('Ancienneté', Tags.CP_AGE, unit='mn', fmt='%.2f')
        self.val_list.add('Surface du peak', Tags.CP_PEAK_AREA, fmt='%.2f')
        self.val_list.add('Temps de rétention', Tags.CP_RETENTION_TIME, unit='s', fmt='%.2f')
        self.val_list.add('Pression du gaz', Tags.CP_PRESSURE_GAS, unit='mbar', fmt='%.2f')
        self.val_list.add('Débit du gaz', Tags.CP_FLOW_GAS, unit='ml/h', fmt='%.2f')
